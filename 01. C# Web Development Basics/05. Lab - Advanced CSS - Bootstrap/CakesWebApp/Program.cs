namespace CakesWebApp
{
    ...
    internal class Program
    {
        private static void Main()
        {
            ...
            serverRoutingTable.Routes[HttpRequestMethod.Get]["/cakes/add"] = request => new CakesController().AddCakes(request);
            serverRoutingTable.Routes[HttpRequestMethod.Post]["/cakes/add"] = request => new CakesController().DoAddCakes(request);
            // cakes/view?id=1
            serverRoutingTable.Routes[HttpRequestMethod.Get]["/cakes/view"] = request => new CakesController().ById(request);
